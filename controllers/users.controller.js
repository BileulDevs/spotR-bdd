...
exports.updateUserPassword = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ success: true, message: 'Mot de passe mis à jour avec succès' });
  } catch (error) {
  ...
  try {
    ...
    if (password || confirmPassword || currentPassword) {
      if (!password || !confirmPassword || !currentPassword) {
        return res
          .status(400)
          .json({ message: 'Tous les champs de mot de passe sont requis' });
      }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    logger.error(
      `Erreur lors de la vérification de l'email pour id : ${req.params.id}`
    );
...
exports.getUserPosts = async (req, res) => {
  try {
    ...
    logger.error(
      `Erreur lors de la récupération des posts pour : ${req.params.id}`
    );
    res.status(500).send({ message: `Error : ${err.message}` });
  }
};
