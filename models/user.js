...
userSchema.pre(
  ...
  async function (next) {
    try {
      ...
      console.log(
        `Subscriptions supprimées pour l'utilisateur ${this.username}`
      );
...
userSchema.pre('findOneAndDelete', async function (next) {
  try {
    ...
    if (user) {
      ...
      console.log(
        `Subscriptions supprimées pour l'utilisateur ${user.username}`
      );
    }
    next();
  } catch (error) {
...
userSchema.pre('deleteMany', async function (next) {
  try {
    ...
    if (userIds.length > 0) {
      ...
      console.log(
        `Subscriptions supprimées pour ${userIds.length} utilisateurs`
      );
    }
